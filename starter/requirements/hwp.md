...
- 앱의 결과를 HWP로 다운로드하거나 Google Drive에 HWP로 저장해야 할 때
...
3. 만든 파일은 사용자가 다운로드하거나 Drive에 올립니다. 처리가 모두 브라우저 안에서 끝나므로 이용자 정보를 앱 서버로 보내지 않습니다(Drive에 올리면 Google에는 저장됩니다).
4. 반복되는 표(참석자 명단, 회기별 기록)에 값을 넣을 때는 누름틀 반복 필드를 쓸지 표 셀 좌표로 쓸지 rhwp 문서에서 확인합니다.
...
| 경로 | HWP 생성 | 비고 |
|---|---|---|
| B. 로컬 퍼스트 | 브라우저에서 `@rhwp/core`로 생성 | 서버가 없어 가장 단순합니다. 다운로드로 끝납니다 |
...
- **Drive에서 HWP 열기·편집·저장**은 rhwp-studio가 이미 합니다. Google Workspace Marketplace에 등록되어 있어 기관 관리자가 설치하면 직원 전체가 Drive에서 "연결 앱으로 열기"로 씁니다. 로컬 HWP와 웹에서 다운로드한 HWP를 브라우저에서 바로 열고 편집하고 인쇄하는 것은 rhwp의 [크롬 확장](https://chromewebstore.google.com/detail/pgakpjflombjmehnebnbpnalhegaanag)과 엣지·파이어폭스 확장이 합니다. 이 두 가지는 앱에 넣지 않고 안내만 합니다.
...
- **기관이 Workspace를 쓰면** Google Cloud의 OAuth 동의 화면을 "내부용"으로 설정할 수 있습니다. 내부용 앱은 기관 계정만 로그인할 수 있고 Google의 심사를 받지 않으므로 Drive 전체를 다루는 범위도 쓸 수 있습니다. 경로 C에서는 가장 간단한 방법입니다.
- **Google Docs 문서는 앱 안에서 편집할 수 없습니다.** Google이 편집 화면을 다른 사이트에 넣는 것을 허용하지 않습니다. 새 탭으로 열어 줍니다. 파일 목록, 만들기, 이동, 삭제는 Drive API나 GAS의 DriveApp으로 합니다.
